...
import aTEAM
import aTEAM.utils
from aTEAM.nn.modules import FD1 as FD
import expr_ex21d1 as expr
...
class POLYPDE2D(torch.nn.Module):
    def __init__(self, dt, dx, kernel_size, max_order, constraint, channel_names, hidden_layers=2, scheme='upwind'):
        """
          Initialize the POLYPDE2D model.

          Args:
              dt (float): Time step.
              dx (float): Spatial step size.
              kernel_size (int): Size of the convolutional kernel.
              max_order (int): Maximum order for finite difference schemes.
              constraint (str or int): Type of constraint for finite difference schemes.
              channel_names (str): Names of the channels.
              hidden_layers (int): Number of hidden layers for the polynomial.
              scheme (str): Convective scheme ('upwind' or 'central').
          """
    ...
    def expr_params(self):
        ...
        Returns:
            list: List of expression parameters.
        """
        parameters = []
    ...
    def diff_params(self):
        """
        Get differential parameters.

        Returns:
            list: List of differential parameters.
        """
        params = []
        for fd in self.fds:
    ...
    def _aggregate_kernel(self):
        """
        Aggregate convolutional kernels.

        Returns:
            tuple: Tuple of aggregated convolutional kernels.
        """
    ...
    def RightHandItems(self, u):
        """
        Compute right-hand side items of ut.

        Args:
            u (torch.Tensor): Input data.
    ...
    #extract values of library candidates for L2 norm and sparsity regression    
    def channel_values(self, u):
        k00,k01,k10,k01_flip,k10_flip,kernels = self._aggregate_kernel()
        assert u.dim() == 4
        conv2d = torch.nn.functional.conv2d
        U = u.split(1,dim=1)
    ...
    def predict_val(self, init, T):
        return self.forward_val(init, T)
    
    def forward_val(self, init, T):
        stepnum = round(T/self.dt.item())
        if abs(stepnum-T/self.dt.item())>1e-5:
            raise NotImplementedError('stepnum=T/self.dt.item() should be an integer, but got T={:f} while self.dt={:f}'.format(T,self.dt.item()))
        return self.multistep_val(init, stepnum)

    def multistep_val(self, init, stepnum):
        if init.dim() == 2:
            assert self.channel_num == 1
            u = init[newaxis,newaxis,...]
        elif init.dim() == 3:
        ...
        for i in range(stepnum-1):
            uadd = self.RightHandItems(u)
            u = u+self.dt*uadd
        channel_values=self.channel_values(u)
        uadd = self.RightHandItems(u)
        return channel_values, uadd
