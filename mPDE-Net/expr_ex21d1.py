...
# Expose 'poly' class in the module's namespace
__all__ = ['poly',]


# Definition of the 'poly' class, which inherits from torch.nn.Module
...
class poly(torch.nn.Module):
    def __init__(self, hidden_layers, channel_num, channel_names=None, normalization_weight=None):
        ...
        # Create layers with linear transformations
        layer = []
    ...
    @property
    def channels(self):
        print("expr self.channel_names", self.channel_names)
        channels = sympy.symbols(self.channel_names)
        print("expr channels", channels)
        return channels
    ...
    def expression(self, calprec=6, eng=None, isexpand=True):
        if eng is None:
            ...
            for k in range(2,4):#fractional terms
                weight,bias = self._cast2symbol(self.layer[k])
                o = weight*channels.transpose()+bias
                if isexpand:
                    o[0] = self._sympychop(o[0], calprec)
            ...
            for k in range(4,8):#nonlinear terms * linear terms
                weight,bias = self._cast2symbol(self.layer[k])
                o = weight*channels.transpose()+bias
                if isexpand:
                    o[0] = self._sympychop(o[0], calprec)
    ...
    def forward(self, inputs):
        ...
        for k in range(4,8):# nonlinear*linear
            o = self.layer[k](outputs)
            outputs = torch.cat([outputs,o[...,:1]*o[...,1:]], dim=-1) 
