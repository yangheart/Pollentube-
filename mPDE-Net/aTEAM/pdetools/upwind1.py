...
from aTEAM.nn.functional import utils as aF
...
def _pad(inputs, pad, mode='wrap', value=0):
    if mode == 'wrap':
        return aF.neumannpad(inputs, pad)# need to change periodic to naumman boundary
    else:
        return torch.nn.functional.pad(inputs, pad, mode, value)
def _pad1d(inputs, pad, mode='wrap', value=0):
    if mode == 'wrap':
        return aF.neumannpad1d(inputs, pad)# need to change periodic to naumman boundary
    else:
        return torch.nn.functional.pad(inputs, pad, mode, value)
